...
def tts(text, title, voice):
    transcription_id = load_trans_id("trans_id.txt")
...
with gr.Blocks() as app:
    ...
    with gr.Tabs():
        with gr.TabItem("TTS"):
            ...
            # Fifth Column
            with gr.Row():
                url_button = gr.Button("Audio URL")
                url_button.click(url, outputs=[output, audio])

        with gr.TabItem("Cloning"):
            with gr.Row():
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        vc_transform0 = gr.Number(
                            label=i18n("Pitch Shift (Integer, Semitone Quantity)"),
                            value=0
                        )
                        input_audio0 = gr.Textbox(
                            label=i18n("Input Path for the Audio to be Processed"),
                            value="input/API/API-audio.mp3",
                        )
                        f0method0 = gr.Radio(
                            label=i18n("Choose the Pitch Extraction Algorithm"),
                        ...
                        filter_radius0 = gr.Slider(
                            minimum=0,
                            maximum=7,
                            label=i18n("If >= 3, use median filtering on the pitch recognition results from Harvest to weaken the mute"),
                            value=3,
                            step=1,
                            interactive=True,
                        )
                    ...
                    with gr.Column():
                        ...
                        file_index1 = gr.Textbox(
                            label=i18n("Path to Feature Index Library File, leave blank to use the dropdown selection result"),
                            value="",
                            visible=False,
                            interactive=True,
                        )
                        file_index2 = gr.Dropdown(
                            label=i18n("Automatically Detect Index Path, dropdown selection"),
                            choices=sorted(index_paths),
                            interactive=True,
                        )
                        ...
                        index_rate1 = gr.Slider(
                            minimum=0,
                            maximum=1,
                            label=i18n("Feature retrieval rate"),
                            value=0.75,
                            interactive=True,
                        )
                        ...
                        protect0 = gr.Slider(
                            minimum=0,
                            maximum=0.5,
                            label=i18n("Protect Clean Consonants and Breaths, Preventing Artifacts like Electric Voice Tearing. Set to 0.5 to disable, lower values increase protection but may reduce indexing effectiveness"),
                            value=0.33,
                            step=0.01,
                            interactive=True,
                        )
                        ...
                        rms_mix_rate0 = gr.Slider(
                            minimum=0,
                            maximum=1,
                            label=i18n("Input Source Volume Envelope Replacement Output Volume Envelope Fusion Ratio, closer to 1 uses output envelope more"),
                            value=0.25,
                            visible=True,
                            interactive=True,
                        )
                        ...
                        resample_sr0 = gr.Slider(
                            minimum=0,
                            maximum=48000,
                            label=i18n("Resample to the Final Sampling Rate after Post-Processing, 0 for no resampling"),
                            value=0,
                            visible=False,
                            step=1,
                            interactive=True,
                    ...
                    #f0_file = gr.File(label=i18n("F0 Curve File, optional. One pitch per line, replace the default F0 and pitch modulation"))
                    but0 = gr.Button(i18n("Convert"), variant="primary")
                    with gr.Row():
                        vc_output1 = gr.Textbox(label=i18n("Output Information"))
                        vc_output2 = gr.Audio(label=i18n("Output Audio (Click the three dots in the lower right corner to download)"))
                    but0.click(
                        vc_single,
                        [
                            spk_item,
            ...
            with gr.Tabs():
                with gr.Row():
                    with gr.Column():
                        ...
                        f0method1 = gr.Radio(
                            label=i18n("Choose the Pitch Extraction Algorithm, 'pm' for Praat's Method, 'harvest' for Harvest, 'crepe' for Crepe (Requires GPU)"),
                            choices=["pm", "harvest", "crepe"],
                            value="harvest",
                            interactive=True,
                        )
                    ...
                    with gr.Column():
                        dir_input = gr.Textbox(
                            label=i18n("Input Path for the Folder Containing Audio Files (Copy from File Explorer Address Bar)"),
                            value="input/.wav",
                        )
                        inputs = gr.File(
                            file_count="multiple", label=i18n("Or Upload Multiple Audio Files, Choose One, Priority to Folder Input")
                        )
                    with gr.Row():
                        format1 = gr.Radio(
